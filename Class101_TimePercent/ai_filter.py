...
def filtered_by_basic_lstm(dataset, ai_settings):
    ...
    # 실전에서 사용할 때는 close가 실제 종가지만, 예측으로 사용할 때는 close가 예측 그래프에서의 종가이다. 
...
def ai_filter(ai_filter_num, engine, until=datetime.datetime.today()):
    if ai_filter_num == 1:
        ai_settings = {
            ...
            "ratio_cut": 3,  # 단위:(%) lookup_step 기간 뒤 ratio_cut(%) 만큼 증가 할 것이 예측 된다면 매수 (ratio_cut 보다 낮을 것 같다면 필터링)
        ...
        try:
            # DISTINCT 는 중복된 데이터를 제거하고 유니크한 데이터만 가져오는 키워드이다.
        ...
        for code_name, in buy_list:
            ...
            if len(df) < 1000:
                ...
                print("테스트 데이터가 적어서 realtime_daily_buy_list 에서 제외")
                continue
            try:
            ...
            if filtered:
                print("기준에 부합하지 않으므로 realtime_daily_buy_list 에서 제외")
                filtered_list.append(code_name)
